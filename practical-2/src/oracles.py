...
class LogRegL2Oracle(BaseSmoothOracle):
    ...
    def hess_vec(self, x, v):
        y = self.b * self.matvec_Ax(x)
        d = self.b ** 2 / self.b.size / (1 / scipy.special.expit(y) + 1 / scipy.special.expit(-y))
        return self.matvec_ATx(d * self.matvec_Ax(v)) + self.regcoef * v
...
def hess_vec_finite_diff(func, x, v, eps=1e-5):
    ...
    return np.array([(func(x + (v + eye[i]) * eps) - func(x + v * eps) -
                       func(x + eye[i] * eps) + func(x)) / eps / eps for i in range(x.size)])