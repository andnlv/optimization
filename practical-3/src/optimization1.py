...
class LineSearchTool(object):
    ...
    def line_search(self, oracle, x_k, d_k, previous_alpha=None):
        ...
        if self._method == 'Wolfe':
            wolfe_result = scipy.optimize.linesearch.scalar_search_wolfe2(
        ...
        while oracle.func_directional(x_k, d_k, alpha) > oracle.func_directional(x_k, d_k, 0) + self.c1*alpha*grad_0:
            alpha = alpha / 2
        return alpha
    ...
    try:
        for i in range(max_iter+1):
            ...
            if display:
                print(i)
            if trace:
                history['time'].append((datetime.now() - time_begin).microseconds/1000)
...
def newton(oracle, x_0, tolerance=1e-5, max_iter=100,
    ...
    trace : bool
        If True, the progress information is appended into history dictionary during training.
        Otherwise None is returned instead of history.ц
    display : bool
        If True, debug information is displayed during optimization.
    ...
    try:
        for i in range(max_iter+1):
            ...
            if np.linalg.norm(grad) ** 2 <= tolerance:
                return x_k, 'success', history
            A_cho = scipy.linalg.cholesky(oracle.hess(x_k))
            d_k = -scipy.linalg.solve_triangular(
                A_cho, scipy.linalg.solve_triangular(A_cho.T, grad, lower=True))
